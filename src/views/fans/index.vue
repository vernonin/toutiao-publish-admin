...
export default {
  ...
  mounted () {
    ...
    function createFormatter (moveTime, moveDistance, avgEnergy, avgSpeed) {
      ...
      const time = (hour < 10 ? '0' + hour : hour) + ':' + (minite < 10 ? '0' + minite : minite)
    ...
    setInterval(function () {
      const speed = generateRandom(70, 80, 0)
      option.series[1].data[0].value = speed
      option.series[3].data[0].value = speed
      const rotatingSpeed = generateRandom(2.5, 3, 1)
      ...
      option.series[4].detail.formatter = createFormatter(minite, moveDistance.toFixed(1), avgEnergy.toFixed(0),
        avgSpeed.toFixed(0))
      myChart.setOption(option, true)
    }, 2000)
